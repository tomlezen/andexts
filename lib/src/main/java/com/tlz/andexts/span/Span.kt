...
class Span internal constructor(private val ctx: Context) {
    ...
    fun quoteColor(@ColorInt color: Int, @IntRange(from = 1L) stripeWidth: Int = 2, @IntRange(from = 0L) gapWidth: Int = 2) {
    ...
    fun bullet(@ColorInt color: Int = 0, @IntRange(from = 0L) radius: Int = 3, @IntRange(from = 0L) gapWidth: Int) {
    ...
    private fun updateImage() {
        ...
        val end = start + 5
        when {
            imageBitmap != null -> builder.setSpan(CustomImageSpan(imageBitmap!!, alignImage), start, end, flag)
            imageDrawable != null -> builder.setSpan(CustomImageSpan(imageDrawable!!, alignImage), start, end, flag)
            imageUri != null -> builder.setSpan(CustomImageSpan(imageUri!!, alignImage), start, end, flag)
            imageResourceId != -1 -> builder.setSpan(CustomImageSpan(imageResourceId, alignImage), start, end, flag)
        }
    }
    ...
    class CustomLineHeightSpan(private val height: Int, private val verticalAlignment: Int) : CharacterStyle(), LineHeightSpan {
    ...
    class SpaceSpan(private val width: Int, private val color: Int = Color.TRANSPARENT) : ReplacementSpan() {

        override fun getSize(paint: Paint, text: CharSequence?, start: Int, end: Int, fm: FontMetricsInt?): Int = width
    ...
    internal class CustomQuoteSpan constructor(private val color: Int, private val stripeWidth: Int, private val gapWidth: Int) : LeadingMarginSpan {
    ...
    internal class CustomBulletSpan constructor(private val color: Int, private val radius: Int, private val gapWidth: Int) : LeadingMarginSpan {
    ...
    internal inner class CustomImageSpan : CustomDynamicDrawableSpan {
        ...
        override var drawable: Drawable? = null
            get() = field ?: if (contentUri != null) {
                try {
                    ctx.contentResolver.openInputStream(contentUri).use { ins ->
                        field = BitmapDrawable(ctx.resources, BitmapFactory.decodeStream(ins))
                        field?.run {
                ...
                try {
                    ...
                    Log.e("Span", "Unable to find resource: $resourceId")
                }
                field
            }
    ...
    internal class ShaderSpan constructor(private val mShader: Shader)
...
fun spannableStringBuilder(ctx: Context, init: Span.() -> Unit): Span.Creator {
    val span = Span(ctx).apply(init)
