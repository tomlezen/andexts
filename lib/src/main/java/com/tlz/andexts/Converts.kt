...
fun Byte.toHexString() = if (Integer.toHexString(0xff and this.toInt()).length == 1) {
...
fun Long.toDate(pattern: String = "yyyy-MM-dd HH:mm:ss", locale: Locale = Locale.CHINA): String {
    return try {
...
fun String.toDate(pattern: String = "yyyy-MM-dd HH:mm:ss", locale: Locale = Locale.CHINA): Date {
    return try {
...
fun Bitmap.toDrawble(): Drawable = BitmapDrawable(this)